...
void makeini()
{
    ...
    buffer ="Pour verifier les fichiers PKG a la racine du support usb et eviter la copie de interne vers externe decommentez la ligne ci-dessous.\r\nnmais rappelez vous que ceci deplacera le pkg de la racine vers le dossier PS4.\r\n//CHECK_USB\r\n\r\nPour renommer les fichiers pkg precedemment lies au nouveau format, decommentez la ligne ci-dessous.\r\n//RENAME_APP\r\n\r\nPour laisser les mises \x83 jour du jeu sur le disque interne, decommentez la ligne ci-dessous.\r\n//IGNORE_UPDATES\r\n\r\nPour utiliser cette liste comme une liste de jeux que vous voulez deplacer, n'ignorez pas, puis decommentez la ligne ci-dessous.\r\n//MODE_MOVE\r\n\r\nExemple ignorer ou deplacer.\r\n\r\nCUSAXXXX1\r\nCUSAXXXX2\r\nCUSAXXXX3";
    ...
    if (!symlink_exists(sourcepath))
    {
        if (isfpkg(sourcepath) == 0) 
        {
            ...
            else
            {
                ...
                {
                    sprintf(cmsg, "%s\n%s\nÉcrasement car les pkg ne correspondent pas", "pkg trouvé a " , ndestpath);
                    systemMessage(cmsg);
                    copyFile(sourcepath, ndestpath);
                } 
                else
                {  
                    sprintf(cmsg, "%s\n%s\nSauter la copie et lier les pkg existants",  "pkg trouvé a " , ndestpath);
                    systemMessage(cmsg);
                    sceKernelSleep(5);
                    unlink(sourcepath);
	...
	while (nthread_run)
	{
		if (isxfer)
		{
			...
			if ((t2 - t1) >= 20)
			{
				...
				if (tmpcnt >= 1048576)
				{
				   sprintf(notify_buf, "Copie: %s\n\n%u%% terminée\nVitesse: %u MB/s", cfile , xfer_pct, tmpcnt / 1048576);
				}
				else if (tmpcnt >= 1024)
				{
				   sprintf(notify_buf, "Copie: %s\n\n%u%% terminée\nVitesse: %u KB/s", cfile , xfer_pct, tmpcnt / 1024);
				}
				else
				{
				   sprintf(notify_buf, "Copie: %s\n\n%u%% terminée\nVitesse: %u B/s", cfile , xfer_pct, tmpcnt);
				}
...
int _main(struct thread *td) {
   ...
   DIR *dir;
	...
	scePthreadCreate(&sthread, NULL, sthread_func, NULL, "sthread");
        ...
        systemMessage("AppToUsb v16 full FR by UGZ Team\n\nMerci à Stooged pour ses travaux sur ce payload");
        systemMessage("Attention, ce payload modifiera le système de fichiers de votre PS4\n\nDébranchez votre lecteur usb pour annuler cette operation");
        sceKernelSleep(10);

        systemMessage("Dernier avertissement\n\nDébranchez votre lecteur usb pour annuler cette operation");
        sceKernelSleep(10);
    ...
    if (usb_mnt_path != NULL)
    {
            ...
            systemMessage("Copie de Apps vers l'USB0\n\nCela peut prendre un certain temps si vous avez beaucoup de jeux installés");
            ...
            if (!dir_exists(tmppathp)) 
            {
               ...
               systemMessage("Copie des mises à jour vers l'USB0");
            ...
            if (!isnometa())
            {
               systemMessage("Traitement appmeta");
            ...
            systemMessage("Terminé.");
    }
    else
    {

            systemMessage("Aucun support usb trouvé.\nVous devez utiliser un support usb formaté en exFAT\nLe lecteur USB doit être branché sur le port USB0");
    }
    nthread_run = 0;
    return 0;
}
